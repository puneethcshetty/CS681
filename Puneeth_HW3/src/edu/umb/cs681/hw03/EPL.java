...
public class EPL {
	...
	public static void main(String args[]) {
		...
		Integer maxMatchesPlayed = eplTable.stream().map((EPL epl)-> epl.getMatchesPlayed()).reduce(0, (result, matchesPlayed)->{
			if(result==0) 
				return matchesPlayed;
		...
		//Maximum matches won
		Integer maxWon = eplTable.stream().map((EPL epl) -> epl.getWon()).reduce(0,(result, won)->{
			if(result==0)
				return won;
			else if(won > result)
				return won;
		...
		//Total teams
		Integer count = eplTable.stream().map((EPL car) -> car.getWon()).reduce(0, (result, carPrice)->{
			return ++result;});
		
		//Maximum points
		Integer maxPoints = eplTable.stream().map((EPL epl) -> epl.getPoints()).reduce(0,(result, points)->{
			if(result==0)
				return points;
